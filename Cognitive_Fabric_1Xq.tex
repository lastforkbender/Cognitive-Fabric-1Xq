...
coalition; a tempered log-pool identity and prior-neutrality result;
...
\]
Spline axes are finite and nondecreasing, and the closed right endpoint is
defined as the final basis vector.  Thus the basis partitions unity throughout
the declared closed domain.
...
$\bar\pi_c=\operatorname{saf}(\pi_c)$ and
$\bar q_{sc}=\clip{q_{sc}}{\epsilon}{1-\epsilon}$, define
\[
G_c=\log\bar\pi_c+
\sum_{s\in K^\star}
w_s(\log\bar q_{sc}-\log\bar\pi_c),
\]
\[
P(c\mid x,X)=
...
P(c\mid x,X)\propto
\bar\pi_c^{\,1-W}
\prod_{s\in K^\star}\bar q_{sc}^{\,w_s}.
\]
...
Expanding $G_c$ gives
\[
G_c=(1-W)\log\bar\pi_c
+\sum_sw_s\log\bar q_{sc}.
\]
Exponentiation and normalization yield the result.
\end{proof}

\begin{corollary}[Prior neutrality]
\label{cor:prior-neutrality}
If every selected local report equals $\pi$, then $P=\pi$.
\end{corollary}

\begin{proof}
Substitute $\bar q_{sc}=\bar\pi_c$ into $G_c$.  Every evidence difference is
zero, so softmax normalizes $\log\pi_c$ back to $\pi_c$.
\end{proof}
...
\]
where $r\in[0,1]$ is reward, $b$ success, $\lambda\ge0$ latency, $t_o$ observed
time, $D$ the originating decision, $m$ the model identifier, and $h$ the
...
For a well-formed snapshot, authentic outcomes, and learning rates in $[0,1]$:
...
\begin{enumerate}[leftmargin=*,itemsep=1pt]
  ...
  realization;
  \item finite inputs and validated constructors;
...
\begin{itemize}[leftmargin=*,itemsep=1pt]
  ...
  \item B-spline nonnegativity and partition of unity on the closed domain;
  ...
  enumeration;
  \item prior neutrality of the tempered log pool;
